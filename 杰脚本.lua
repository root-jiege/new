...
game.Players.LocalPlayer:Kick("请到230930606获取最新脚本")
end
...
--
local RZCQ1 = win:Tab("杰者传奇",'6035145364')
--
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
local UITab = UITab1:section("『☯杰脚本公告☯』",true)
--
...
--
local RZCQ = RZCQ1:section("杰者传奇",false)
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      DoorsTab:Button(" 杰  -- Doors", function()
...
local function donate(plr, amount)
    ...
    notify('杰', '转钱' .. tostring(amount) .. ' 给 ' .. tostring(plr), 4);
    scr2.exitNotice();
end
...
Section:Button("获取当前死亡后加载时间", function()
    bailib:Notification('杰', " 时间为" .. bai.loaddupeaxewaittime .. "秒", "好")
end)
...
Sectionhuanjin:Button("船离开时间", function()
    ...
    if game.Workspace.Ferry.TimeToDeparture.Value == 0 then
        ...
        notify('杰', "距离船离开时间有" .. game.Workspace.Ferry.TimeToDeparture.Value .. "秒", 4)
    end
end)
Sectionhuanjin:Button("桥上升时间", function()
    ...
    if game.Workspace.Bridge.DownTime.Value < 0 then
        ...
        notify('杰', "距离桥上升的时间有" .. game.Workspace.Bridge.DownTime.Value .. "秒", 4)
    end
end)
Sectionhuanjin:Button("查看游戏时间", function()
    notify('杰', "游戏时间为" .. tostring(game.Lighting.TimeOfDay):sub(1, -2):sub(1, -2):sub(1, -2), 4)
end)
...
FM:Button("bark2.0", function()
 ...
 FM:Button(" 杰  -- 伐木大亨2(盗版浮光掠影)", function()
...
if REN["白名单"]["开发者白名单"]["状态"] == true then
        ...
        if REN["限免"] == true then
            notify(" 杰 ","现在是限免时间，无需白名单",3)
            wait(1)
            notify(" 杰 ","正在启动脚本....",5)
            wait(3)       
            scripts()
        else
        if R == "杰脚本交流群:230930606" then
            notify(" 杰 ","保护函数正确",3)
        else   
            setclipboard("230930606")
            game.Players.LocalPlayer:Kick("请到230930606获取最新脚本")                                           
        end
        if REN["白名单"]["普通白名单"]["状态"] == true then
            ...
            scripts()
                ...
                setclipboard("230930606")
            game.Players.LocalPlayer:Kick(REN["游戏名"].."请您到230930606购买白名单\n")
        end
    end
end
